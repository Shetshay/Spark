...
function get_mysqli_connection() {
    ...
    if (!isset($connection)) {
        $connection = mysqli_connect(
            ...
            'sparksocial', // the MariaDB username
            'laicoskraps3420S23', // the MariaDB username's password
            'sparksocial' // the MariaDB database name
...
function get_pdo_connection() {
    ...
    if (!isset($conn)) {
        try {
            ...
            $conn = new PDO(
                "mysql:host=localhost;dbname=sparksocial",  // change dbname
                "sparksocial",                          // change username
                "laicoskraps3420S23",                      // change password
                $options);
        }
